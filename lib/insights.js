 ...
 */
export function projection(userId, month = currentMonth()) {
 ...
 */
export function buildInsights(userId, month = currentMonth()) {
  const out = []
  const { total, count } = monthTotal(userId, month)
  if (!count) return out

  // 1. Comparación con el mes anterior.
  const anterior = monthTotal(userId, previousMonth(month))
  if (anterior.count) {
  ...
  const categorias = totalByCategory(userId, month)
  ...
  const proy = projection(userId, month)
  ...
  const dias = totalByDay(userId, month)
  if (dias.length > 2) {
